...
// ════════════════════════════════════════════════════════
//  SENAI – Reserva de Notebooks | app.js
// ════════════════════════════════════════════════════════
...
// null = não verificado, true = API ok, false = API com falha
let _apiOnline = null;
let _apiErro   = '';
...
async function fetchAll(force = false) {
  ...
  try {
    ...
    if (!Array.isArray(data)) {
      // Servidor retornou erro (ex: Code.gs com problema ou não implantado)
      const msg = data.error || JSON.stringify(data);
      console.error('[API] fetchAll — resposta inválida:', msg);
      _apiOnline = false;
      _apiErro   = 'Planilha retornou erro: ' + msg;
      // usa cache local como fallback sem sobrescrever o bom cache existente
      if (_cache === null) {
        _autoReturnLocal();
        _cache = JSON.parse(localStorage.getItem(STORE_KEY) || '[]');
      }
...
async function saveReserva(reserva) {
  ...
  try {
    ...
    throw err; // repropaga para handleSubmit mostrar mensagem ao usuário
  }
  ...
  // Salvo com sucesso: adiciona ao cache local imediatamente
...
// ─── RETORNO AUTOMÁTICO (localStorage) ──────────────────
...
function getDisp(unidade, slot) {
  const usado = getAll()
    .filter(r => r.status === 'ativa' && r.unidade === unidade)
    .filter(r => r.slots.some(s => overlaps(s, slot)))
    .reduce((n, r) => n + r.quantidade, 0);
  return MAX_NB - usado;
}

// Calcula o pico simultâneo de notebooks em uso num dia inteiro
function peakUsage(unidade, dateISO) {
  ...
  // Fins (-1) antes de inícios (+1) no mesmo instante (evita dupla contagem)
...
// Sugere datas próximas com o mesmo horário e duração que estejam disponíveis
function findNearbyDates(unidade, slot, quantidade, maxResults = 3) {
  ...
  for (let i = 1; i <= 30 && suggestions.length < maxResults; i++) {
    ...
    // Descarta se ultrapassar meia-noite quando o original não ultrapassa
    ...
    const disp     = getDisp(unidade, candSlot);
    if (disp >= quantidade) {
      suggestions.push({
        slot:  candSlot,
        disp,
...
// Formata um ISO completo com timezone (ex.: "2025-06-17T13:00:00.000Z",
// usado nos timestamps criadoEm/devolvidoEm) — diferente de fmtDateShort,
// que espera o formato local truncado "YYYY-MM-DDTHH:MM".
...
function updateQtyHint() {
  ...
  if (_cache === null || !unidade || slots.length === 0) {
    hint.textContent = 'Selecione a unidade e adicione um período para ver disponibilidade';
    hint.className   = 'qty-hint';
    return;
  }

  const minDisp = slots.reduce((min, s) => Math.min(min, getDisp(unidade, s)), MAX_NB);
...
// ─── RESERVA RECORRENTE ─────────────────────────────────
// Gera vários períodos de uma vez (mesmo horário) nos dias da semana
// escolhidos, entre uma data início e uma data fim.
...
async function handleSubmit(e) {
  ...
  if (!matricula) {
    fieldError('wrap-matricula', 'err-matricula', 'Informe sua matrícula.');
    valid = false;
  }
  if (!unidade) {
    fieldError('wrap-unidade', 'err-unidade', 'Selecione a unidade.');
    valid = false;
  }
  ...
  for (const slot of slots) {
    const disp = getDisp(unidade, slot);
    if (disp < quantidade) conflicts.push({ slot, disp });
  }

  if (conflicts.length > 0) {
    const first       = conflicts[0];
    const suggestions = findNearbyDates(unidade, first.slot, quantidade);
...
function showConfirmation(reserva) {
  ...
  document.getElementById('conf-matricula').textContent = maskMat(reserva.matricula);
  ...
  // Atualiza disponibilidade com os dados já no cache (otimista)
  renderSituacao();
}
...
async function renderConsulta() {
  ...
  const unidade = document.getElementById('cons-unidade').value;
  ...
  function avail(u, d) {
    if (hasFilter) return getDisp(u, { retirada: `${d}T${horaRet}`, devolucao: `${d}T${horaDev}` });
    return MAX_NB - peakUsage(u, d);
  }
  ...
  if (unidade) {
    // Visão detalhada de uma unidade com horários reservados
    const rows = weekdays.map(d => {
      const [,, dy] = d.split('-');
      const disp    = Math.max(0, avail(unidade, d));
      const cls     = badgeCls(disp);
      const res     = reservasDoDia(unidade, d);
    ...
    container.innerHTML = `<div class="search-card" style="padding-top:20px">
      <div class="search-eyebrow"><span class="eyebrow-bar"></span><span>${unidade}${timeLabel}</span></div>
    ...
    // Visão comparativa de todas as unidades
    const rows = weekdays.map(d => {
      const [,, dy] = d.split('-');
    ...
    container.innerHTML = `<div class="search-card" style="padding-top:20px">
      <div class="search-eyebrow"><span class="eyebrow-bar"></span><span>Todas as Unidades${timeLabel}</span></div>
      <div class="avail-table-wrap">
        <table class="avail-table">
          <thead><tr>
            <th>Data</th>
            ${UNITS.map(u => `<th>${u}</th>`).join('')}
          </tr></thead>
          <tbody>${rows}</tbody>
        </table>
      </div>${legend}
...
async function renderSituacao() {
  ...
  // fetchAll nunca joga exceção — o status da API fica em _apiOnline/_apiErro
  await fetchAll();
  _updateApiStatusEl();
  ...
  try {
    container.innerHTML = UNITS.map(u => {
      const ativas = getAll().filter(r =>
        r.status === 'ativa' && r.unidade === u && Array.isArray(r.slots)
      );
        ...
        return `<div class="reserva-row ${isNow ? 'reserva-now' : ''}">
          ...
          <div style="flex:1;min-width:0">
            ...
            <div style="margin:3px 0 0;font-size:10.5px;color:var(--gray-60);font-weight:600;line-height:1.4">
              Mat. ${maskMat(r.matricula || '')} &nbsp;·&nbsp; ${r.quantidade} notebook${r.quantidade > 1 ? 's' : ''} &nbsp;·&nbsp; ${fmtTime(slot.retirada)}–${fmtTime(slot.devolucao)}
            </div>
          </div>
          ${isNow
...
// Senha guardada em memória (nunca em localStorage) após autenticação bem-sucedida,
// enviada ao servidor em toda ação de cancelar/editar — ver senhaValida() no Code.gs.
...
async function exportCSV() {
  ...
  const unidade = document.getElementById('exp-unidade').value;
  const inicio  = document.getElementById('exp-inicio').value;
  const fim     = document.getElementById('exp-fim').value;

  let data = getAll().filter(r => !unidade || r.unidade === unidade);
  ...
  const header = ['ID','Nome','Matrícula','Unidade','Quantidade',
...
// Sessão expirada ou senha inválida no servidor: força novo login na área restrita.
// (o toast do erro já é exibido pelo chamador — handleCancelar/salvarEdicao)
...
async function renderGerenciar() {
  ...
  const unidade = document.getElementById('ger-unidade').value;
  ...
  if (unidade) reservas = reservas.filter(r => r.unidade === unidade);
  if (inicio || fim) {
...
function gerItemHTML(r) {
  ...
  return `<div class="ger-item" data-id="${r.id}">
    <div class="ger-item-info">
      ...
      <div class="ger-item-meta">${r.unidade} &middot; Mat. ${r.matricula} &middot; ${r.quantidade} notebook${r.quantidade !== 1 ? 's' : ''}</div>
